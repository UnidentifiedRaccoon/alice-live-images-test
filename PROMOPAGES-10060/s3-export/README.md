...
Этот каталог фиксирует раскладку 17 доступных публикаций из тикета по кабинетам и готовит воспроизводимую загрузку 376 MP4 в `promopages-front-bundles`. Статья 02 недоступна из-за HTTP 404, а два результата Veo имеют финальный статус `provider-filtered`; они сохраняются в отчётах, но файлов для них нет.
...
По умолчанию результат создаётся в `PROMOPAGES-10060/s3-export/output`. Режим `--materialize auto` сначала использует hardlink, а при невозможности делает копию. Это экономит около 2.53 GiB локального места и при этом оставляет в staging обычные файлы, не симлинки. Для самостоятельного переносимого каталога используйте:
...
Источниками служат только финальные `video_path` из двух агрегатных манифестов:
...
| Кабинеты | 9 |
| Доступные статьи | 17 |
| Недоступные статьи | 1 |
| Изображения | 126 |
| Логические результаты | 378 |
| MP4 для загрузки | 376 |
