...
- автоматическое расширение prompt на стороне провайдера;
...
`alibaba/wan-2.2` намеренно не входит в первую версию: это legacy-маршрут, для
которого сейчас нет подтверждённого активного provider-контракта и управляемого
prompt expansion.
...
Список `model_id` опционален: можно запросить одну или обе поддерживаемые
модели. Если список не указан, подготовь варианты для обеих моделей v1.
