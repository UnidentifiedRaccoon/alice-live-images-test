...
Один beat не означает неподвижного субъекта или движения только одной части
тела. Причинно синхронизированная выразительная реакция — например, один
непрерывный злой выкрик с одновременной тряской уже поднятых рук — остаётся
одним действием, если не добавляет второй жест, новую сцену или эмоциональный
поворот. Ограничение вида «персонаж не должен стать добрым» фиксирует
эмоциональный знак, а не исходную мимику и позу покадрово: не подавляй явно
запрошенную энергичность микродвижением или статичным hold.
...
Сохраняй естественную абсолютную длительность знакомого физического действия
между моделями. Более длинный target расширяет финальный hold, а не замедляет
тот же blink, шаг, падение капли или перенос веса. Если пользователь явно не
просил slow motion, не растягивай микродействие пропорционально общей
длительности клипа.

Общий ритм:

```text
0–10%       exact first-frame continuity; motion starts without a jump
first 2.0 s one primary action runs at natural real-time speed and completes
remainder    deceleration if physically needed, then a stable final hold
```

Для clip короче двух секунд сократи амплитуду и используй model-spec; не пытайся
втиснуть сложное действие. Непрерывный источник вроде водопада, текущей воды
или вращающегося колеса сохраняет нормальную физическую скорость весь клип, а
его локальный читаемый эффект завершается в том же раннем timebox. Более
длинная spec даёт больше времени на hold, но не разрешает второй beat, смену
сцены или вторую камеру. Точные поправки темпа бери из model-spec.

Если сохранённые provider outputs систематически распределяют микродействие по
всему клипу вопреки timebox, не придумывай `speed` или `fps`. Используй только
подтверждённую model-spec native duration и проверь A/B покадрово. Более
короткая duration считается model-specific temporal calibration, а не
постобработкой; она не отменяет ранний deadline и сама по себе не гарантирует
правильную физику.

Если правильное завершённое состояние буквально совпадает с source frame
(например, один blink с возвратом взгляда или locked flat-raster hold), а
выбранная model-spec подтверждает last-frame input, допустимо передать тот же
source как `last_frame`. Явно укажи, что endpoints намеренно одинаковы. Не
используй этот приём для шага, ряби или другого действия с новым финальным
состоянием и не считай совпадение endpoints доказательством статичности всех
промежуточных кадров.

### Контрольный replay одного prompt между моделями

Это отдельный экспериментальный режим только по прямому запросу пользователя.
Не переписывай текст под target-model: возьми уже materialized Positive и
Negative source-run побайтно, запиши `prompt_source_model_id` и передай их тем
же combined runtime-текстом `Positive + "\n\nAvoid: " + Negative`. Для такого
Eliza-run `embed_negative_in_positive` обязателен, а отдельное provider negative
поле не отправляется. Это также позволяет сохранить длинный source Negative,
который не помещается в отдельный лимитированный Wan 2.7 field, не урезая его.

Длительность остаётся нативным параметром target-model и может отличаться от
source-run; она не добавляет новую хореографию. Не обрезай output и не добавляй
duration-suffix без отдельного запроса. Повтори только те frame inputs, которые
были в исходном опыте: отсутствие source `last_frame` означает отсутствие
target `last_frame`. Валидатор обязан отклонять self-reference, несовпадающий
текст и replay без combined transport. В отчёте отдельно укажи обязательные
adapter-конфаундеры вроде Veo `enhancePrompt: true`.
...
2. для динамического действия без user-requested slow motion — `slow motion`,
   `time-stretched motion` или эквивалентный model-specific fragment;
3. исчезновение, появление или трансформация anchors;
4. ошибки хрупких деталей из profile и `scene_tags`;
