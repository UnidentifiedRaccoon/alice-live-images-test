...
Для каждого изображения с resolved routing выбери ровно один `primary_class` из
канонической
[таксономии PROMOPAGES-9857](../../../PROMOPAGES-9857/articles/taxonomy.md) по
доминирующему визуальному сценарию. Если evidence не разрешает границу между
классами, не создавай фиктивное значение: используй описанный ниже runtime
fallback без `primary_class`.
...
Для `text_interface_collage` с resolved graphic routing действует
дополнительная ось routing:
...
У остальных `primary_class`, а также у runtime-входа с unresolved graphic
routing оба поля отсутствуют. В отличие от `scene_tags`,
...
## Граница до graphic routing

Сначала реши, является ли плоская графическая грамматика доминирующим
сценарием. Только после выбора `primary_class: text_interface_collage` разрешено
назначать `graphic_kind`. Наличие текста, логотипа, карточки или нескольких
объектов само по себе не делает кадр графическим.

| Видимое устройство кадра | Primary routing |
| --- | --- |
| Единый фотографический или 3D-мир с общей перспективой, светом и главным человеком, товаром, транспортом или местом | Соответствующий subject profile; текст и branding остаются anchors/risks. |
| Спроектированный промоблок, где headline, CTA, QR, branding и media-region образуют плоскую layout-грамматику | `text_interface_collage`, затем `banner`. |
| Одна непрерывная фотография, даже широкая, с несколькими людьми, предметами или студийным оборудованием | Subject profile; это не `collage`. |
| Независимые панели, вырезки или области с разной пространственной грамматикой и собственными границами | `text_interface_collage`, возможен `collage`. |
| Символический ортогональный план, карта, таблица, chart или node/connector-схема | `text_interface_collage`, затем соответствующий kind. |
| Фотореалистичный вид сверху, aerial render или masterplan с непрерывным пространством | `architecture_place` либо другой subject profile; это не `floor_plan` и не `map`. |

Если эта граница не разрешается уверенно, не выбирай наиболее «похожий» класс.
Зафиксируй primary routing как unresolved и используй общий fallback ниже.
...
### Порог самостоятельности secondary kinds

Добавляй secondary kind только для самостоятельной макроструктуры со своими
preservation anchors. Вложенный текст, label, иконка, thumbnail, декоративная
шкала или обычная карточка внутри более сильной грамматики отдельным kind не
становятся.

| Кандидат | Считай отдельным kind только когда | Не считай отдельным kind когда |
| --- | --- | --- |
| `text_document` | Непрерывный текстовый документ сам является содержанием. | Это labels, headline, подписи, legal line или body copy внутри UI, banner, table, plan или chart. |
| `collage` | Есть минимум две самостоятельные области с явными границами или одна крупная независимая media-панель рядом с другой грамматикой. | Это thumbnails, gallery previews, cards или изображения внутри одного UI state/banner. |
| `chart` | Marks, axes, scale, legend или series кодируют данные и требуют собственных anchors. | Это декоративный gauge, progress-like ornament, иконка или иллюстративная шкала без data semantics. |
| `table` | Строки и столбцы образуют доминирующую data-grid с точными headers и values. | Это список, repeated cards или aligned controls без табличной семантики. |
| `diagram` | Существующие nodes и connectors кодируют отношения или направление. | Это декоративные линии, стрелка-иконка или иллюстрация. |
| `map` | География, markers, route, legend или scale образуют symbolic map grammar. | Это aerial photo, landscape, photoreal masterplan или декоративный фон. |
| `floor_plan` | Стены, комнаты, проёмы и размеры образуют symbolic orthographic plan. | Это top-down photo/render непрерывной территории. |

`banner` и `ui_screenshot` обычно являются сильной контейнерной грамматикой.
Внутренний элемент получает secondary kind только по правилам выше. Поэтому UI
с data-bearing donut может быть `[ui_screenshot, chart]`, а UI с декоративной
шкалой или thumbnails остаётся только `[ui_screenshot]`.
...
совместимое направление пользователя. Валидный intent обязан назвать видимый
target и одно действие, разрешённое policy активного kind. Общие слова
`animate`, `dynamic`, `cinematic`, `parallax` или `bring to life` недостаточны.
Без валидного intent любой графический вид начинается с
`ACT_HOLD_AND_SETTLE`.

### Routing regression fixtures

Эти пары фиксируют boundary-контракт на фактической расширенной выборке. Они не
являются готовыми prompts.

| Fixture | Ожидаемый routing | Обязательное следствие |
| --- | --- | --- |
| `11-ostin-vesenniaia-kapsula/01.jpeg` | `people_action` | Единая studio photo не становится `collage`. |
| `15-ozon-gruzoperevozki/01.png` | `vehicle_mechanism` | Branding не превращает единый 3D still life в `banner`; hold + Module A. |
| `15-ozon-gruzoperevozki/03.png` | `text_interface_collage`; `[banner]` | CTA, QR и headline зафиксированы; truck не продолжает движение. |
| `17-level-michurinskiy-kvartira/04.png` | `text_interface_collage`; `[floor_plan]` | Стены, двери, числа и мебель неизменны. |
| `17-level-michurinskiy-kvartira/07.webp` | `architecture_place` | Photoreal top-down masterplan не становится `map`/`floor_plan`; Module A. |
| `18-dalan-sredstva-dlia-volos/03.png` | `text_interface_collage`; `[ui_screenshot]` | Thumbnails не добавляют `collage`. |
| `18-dalan-sredstva-dlia-volos/07.png` | `text_interface_collage`; `[ui_screenshot, collage]` | UI policy принудительно оставляет обе панели в hold. |
| `19-level-travel-otpusk-v-turcii/02.png` | `text_interface_collage`; `[table]` | Значения, строки, колонки и grid неизменны. |
| `20-sravni-kreditnyi-reiting/03.png` | `text_interface_collage`; `[banner]` | Декоративная шкала не добавляет `chart`. |
| `20-sravni-kreditnyi-reiting/04.png` | `text_interface_collage`; `[ui_screenshot, chart]` | Нет fill/recompute; negative покрывает UI и chart без дублей. |

В текущем dataset нет реального active-route evidence для `map`, `diagram` и
`chart`. Их policies остаются консервативными: без однозначной grammar и явного
видимого target используй hold, а не экспериментальный accent.

## Общий fallback для unresolved primary class

Runtime-вход может остаться без `primary_class`, если два класса действительно
неразрешимы или нужного профиля ещё нет. Не записывай `unresolved` как новый
класс и не выбирай ближайший профиль.

- Primary action: `ACT_HOLD_AND_SETTLE`.
- Secondary motion: `SEC_NONE`.
- Camera: Module A при любом тексте, interface, panel/layout evidence, flat
  geometry или другом хрупком anchor; Module B допустим только для явно
  фотографического кадра без этих рисков.
- Negative: видимые anchors и общий wrong-action risk без class- или
  kind-specific fragments.

Такой fallback исполним без активного scene profile и сохраняет расширяемость:
новый подтверждённый класс позже добавляется отдельным профилем.
...
при видимом источнике; `SEC_ENVIRONMENT` допустим только для уже видимых воды
или листвы в явно фоновой области и остаётся слабее неподвижного товара; иначе
`SEC_NONE`.
...
Для outdoor product дополнительно сохраняй background separation, форму листвы,
waterline/horizon и исходную глубину резкости; фон не становится вторым сюжетом.
...
**Camera.** Base: `E`. Base меняется на `A` для strict top-down masterplan,
aerial geometry или ортогональной композиции. Allowed: `A` для хрупкой
геометрии, текста или симметрии; `C` для пространства с уже выраженным
направлением движения. Forbidden: `B`, `D`.
...
| --- | --- |
| `banner` | `ACT_LOCAL_MEDIA_MICROMOTION` для одного субъекта в ясно отделённой фотообласти без любой наложенной графики, включая текст, CTA, логотип и QR; иначе hold. |
...
| `diagram` | `ACT_GRAPHIC_ACCENT` как мягкий оптический акцент существующего узла или connector; новый signal/token не создаётся, topology неизменна. |
...
затронуть его anchors. Kind с policy «только hold», включая `ui_screenshot` и
`text_document`, принудительно переводит весь mixed-кадр в hold. Если
пересечение пусто, конфликтно или не доказано по плоскому растру, используй
`ACT_HOLD_AND_SETTLE` + `SEC_NONE` + Module A. Не выбирай частично совместимое
действие по одному виду.
...
kind-, tag- и profile-level. Для unresolved graphic routing kind-clauses
отсутствуют: используй только релевантные generic flat-raster failures из этого
списка и видимые anchors.
...
1. Выбери один `primary_class` по визуальному доминанту либо явно зафиксируй
   runtime routing как unresolved и примени общий fallback.
...
4. Для resolved primary routing по состоянию и движению, непосредственно
   видимым в первом кадре, выбери один primary action module. Для unresolved
   routing используй общий `ACT_HOLD_AND_SETTLE`.
5. Добавь secondary-motion modules только из прямых `Motion sources`; для
   unresolved primary routing используй `SEC_NONE`.
6. Назначь ровно один camera module: начни с base camera, переходи к allowed
   camera только при ясной пользе, подтверждённой самим кадром. Module A
   кодирует отсутствие camera motion; тег не может разрешить или запретить
   камеру.
...
8. При неоднозначности внутри resolved профиля используй его `low-risk
   fallback`; для unresolved primary routing используй общий fallback.
