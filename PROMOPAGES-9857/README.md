# PROMOPAGES-9857 — исходные изображения 21 статьи

Набор собран для Side-by-Side-теста Wan 2.2 (Алиса), Wan 2.7 и Veo 3.1 Lite.
Внутри `articles/` находится 21 папка в порядке постановки, `manifest.csv`,
`taxonomy.md` и отчёт независимой проверки.

## Состав

- 21 статья;
- 133 появления изображений;
- 127 уникальных `image_id`;
- 78 JPEG, 54 PNG и 1 WebP;
...
Статья 21 добавляет медицинскую редакционную инфографику и ещё семь исходных
изображений. Для тестового кейса выбрано body-изображение `21/04`; переданный
скриншот используется только для сопоставления, а в наборе хранится untouched
`/orig`.
